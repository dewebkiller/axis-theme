<?php get_header();?>
<div class="circular-text">
    <a href="https://www.youtube.com" target="_blank">
        <svg viewBox="0 0 100 100" width="100" height="100">
            <defs>
                <path id="circle" d="
        M 50, 50
        m -37, 0
        a 37,37 0 1,1 74,0
        a 37,37 0 1,1 -74,0" />
            </defs>
            <text font-size="17">
                <textPath xlink:href="#circle">
                    - Explore More - Explore More
                </textPath>

            </text>
        </svg>
        <span><i class="fa fa-play"></i></span>
    </a>
</div>
<section class="content-wrapper section-padding section-courses">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <h2 class="text-center bg-curved-line color-orange pb-3">Our Courses</h2>
                <ul class="dwk_tabs">
                    <li class="active" rel="dwk_tab1">Popular Courses</li>
                    <li rel="dwk_tab2">Upcoming Courses</li>
                    <li rel="dwk_tab3">Workshop</li>
                </ul>
                <div class="dwk_tab_container">
                    <h3 class="d_active dwk_tab_drawer_heading" rel="dwk_tab1">Popular Courses</h3>
                    <div id="dwk_tab1" class="dwk_tab_content">
                        <div class="swiper mySwiper swiper-popular-courses">
                            <div class="swiper-wrapper">
                                <div class="swiper-slide">
                                    <div class="card">
                                        <img src="<?php echo bloginfo('template_url' );?>/images/course-thumbnail.png" class="card-img-top" alt="...">
                                        <div class="card-body">
                                            <div class="card-subheading">
                                                <img src="<?php echo bloginfo('template_url' );?>/images/students-group-icon.png" alt="">
                                                <span>40+ Students</span>
                                            </div>
                                            <span class="date">1 - 28 July 2022</span>
                                            <h5 class="card-title">Computer Basics</h5>
                                            <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                                            <div class="d-flex align-items-center justify-content-between">
                                                <h6 class="color-orange">Duration: 2 Months</h6>
                                                <a href="#" class="btn btn-primary hvr-bob">Learn More</a>
                                            </div>

                                        </div>
                                    </div>
                                </div>
                                <div class="swiper-slide">
                                    <div class="card">
                                        <img src="<?php echo bloginfo('template_url' );?>/images/course-thumbnail.png" class="card-img-top" alt="...">
                                        <div class="card-body">
                                            <div class="card-subheading">
                                                <img src="<?php echo bloginfo('template_url' );?>/images/students-group-icon.png" alt="">
                                                <span>40+ Students</span>
                                            </div>
                                            <span class="date">1 - 28 July 2022</span>
                                            <h5 class="card-title">Graphic Design</h5>
                                            <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                                            <div class="d-flex align-items-center justify-content-between">
                                                <h6 class="color-orange">Duration: 2 Months</h6>
                                                <a href="#" class="btn btn-primary hvr-bob">Learn More</a>
                                            </div>

                                        </div>
                                    </div>
                                </div>
                                <div class="swiper-slide">
                                    <div class="card">
                                        <img src="<?php echo bloginfo('template_url' );?>/images/course-thumbnail.png" class="card-img-top" alt="...">
                                        <div class="card-body">
                                            <div class="card-subheading">
                                                <img src="<?php echo bloginfo('template_url' );?>/images/students-group-icon.png" alt="">
                                                <span>40+ Students</span>
                                            </div>
                                            <span class="date">1 - 28 July 2022</span>
                                            <h5 class="card-title">Video Editing</h5>
                                            <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                                            <div class="d-flex align-items-center justify-content-between">
                                                <h6 class="color-orange">Duration: 2 Months</h6>
                                                <a href="#" class="btn btn-primary hvr-bob">Learn More</a>
                                            </div>

                                        </div>
                                    </div>
                                </div>
                                <div class="swiper-slide">
                                    <div class="card">
                                        <img src="<?php echo bloginfo('template_url' );?>/images/course-thumbnail.png" class="card-img-top" alt="...">
                                        <div class="card-body">
                                            <div class="card-subheading">
                                                <img src="<?php echo bloginfo('template_url' );?>/images/students-group-icon.png" alt="">
                                                <span>40+ Students</span>
                                            </div>
                                            <span class="date">1 - 28 July 2022</span>
                                            <h5 class="card-title">Motion Graphics</h5>
                                            <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                                            <div class="d-flex align-items-center justify-content-between">
                                                <h6 class="color-orange">Duration: 2 Months</h6>
                                                <a href="#" class="btn btn-primary hvr-bob">Learn More</a>
                                            </div>

                                        </div>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <!-- #tab1 -->
                    <h3 class="dwk_tab_drawer_heading" rel="dwk_tab2">Upcoming Courses</h3>
                    <div id="dwk_tab2" class="dwk_tab_content">
                        <div class="swiper mySwiper swiper-popular-courses">
                            <div class="swiper-wrapper">
                                <div class="swiper-slide">
                                    <div class="card">
                                        <img src="<?php echo bloginfo('template_url' );?>/images/course-thumbnail.png" class="card-img-top" alt="...">
                                        <div class="card-body">
                                            <div class="card-subheading">
                                                <img src="<?php echo bloginfo('template_url' );?>/images/students-group-icon.png" alt="">
                                                <span>40+ Students</span>
                                            </div>
                                            <span class="date">1 - 28 July 2022</span>
                                            <h5 class="card-title">Upcoming Courses 1</h5>
                                            <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                                            <div class="d-flex align-items-center justify-content-between">
                                                <h6 class="color-orange">Duration: 2 Months</h6>
                                                <a href="#" class="btn btn-primary hvr-bob">Learn More</a>
                                            </div>

                                        </div>
                                    </div>
                                </div>
                                <div class="swiper-slide">
                                    <div class="card">
                                        <img src="<?php echo bloginfo('template_url' );?>/images/course-thumbnail.png" class="card-img-top" alt="...">
                                        <div class="card-body">
                                            <div class="card-subheading">
                                                <img src="<?php echo bloginfo('template_url' );?>/images/students-group-icon.png" alt="">
                                                <span>40+ Students</span>
                                            </div>
                                            <span class="date">1 - 28 July 2022</span>
                                            <h5 class="card-title">Upcoming Courses 2</h5>
                                            <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                                            <div class="d-flex align-items-center justify-content-between">
                                                <h6 class="color-orange">Duration: 2 Months</h6>
                                                <a href="#" class="btn btn-primary hvr-bob">Learn More</a>
                                            </div>

                                        </div>
                                    </div>
                                </div>
                                <div class="swiper-slide">
                                    <div class="card">
                                        <img src="<?php echo bloginfo('template_url' );?>/images/course-thumbnail.png" class="card-img-top" alt="...">
                                        <div class="card-body">
                                            <div class="card-subheading">
                                                <img src="<?php echo bloginfo('template_url' );?>/images/students-group-icon.png" alt="">
                                                <span>40+ Students</span>
                                            </div>
                                            <span class="date">1 - 28 July 2022</span>
                                            <h5 class="card-title">Upcoming Courses 3</h5>
                                            <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                                            <div class="d-flex align-items-center justify-content-between">
                                                <h6 class="color-orange">Duration: 2 Months</h6>
                                                <a href="#" class="btn btn-primary hvr-bob">Learn More</a>
                                            </div>

                                        </div>
                                    </div>
                                </div>
                                <div class="swiper-slide">
                                    <div class="card">
                                        <img src="<?php echo bloginfo('template_url' );?>/images/course-thumbnail.png" class="card-img-top" alt="...">
                                        <div class="card-body">
                                            <div class="card-subheading">
                                                <img src="<?php echo bloginfo('template_url' );?>/images/students-group-icon.png" alt="">
                                                <span>40+ Students</span>
                                            </div>
                                            <span class="date">1 - 28 July 2022</span>
                                            <h5 class="card-title">Upcoming Courses 4</h5>
                                            <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                                            <div class="d-flex align-items-center justify-content-between">
                                                <h6 class="color-orange">Duration: 2 Months</h6>
                                                <a href="#" class="btn btn-primary hvr-bob">Learn More</a>
                                            </div>

                                        </div>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <!-- #tab2 -->
                    <h3 class="dwk_tab_drawer_heading" rel="dwk_tab3">Workshop</h3>
                    <div id="dwk_tab3" class="dwk_tab_content">
                        <div class="swiper mySwiper swiper-popular-courses">
                            <div class="swiper-wrapper">
                                <div class="swiper-slide">
                                    <div class="card">
                                        <img src="<?php echo bloginfo('template_url' );?>/images/course-thumbnail.png" class="card-img-top" alt="...">
                                        <div class="card-body">
                                            <div class="card-subheading">
                                                <img src="<?php echo bloginfo('template_url' );?>/images/students-group-icon.png" alt="">
                                                <span>40+ Students</span>
                                            </div>
                                            <span class="date">1 - 28 July 2022</span>
                                            <h5 class="card-title">Workshop 1</h5>
                                            <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                                            <div class="d-flex align-items-center justify-content-between">
                                                <h6 class="color-orange">Duration: 2 Months</h6>
                                                <a href="#" class="btn btn-primary hvr-bob">Learn More</a>
                                            </div>

                                        </div>
                                    </div>
                                </div>
                                <div class="swiper-slide">
                                    <div class="card">
                                        <img src="<?php echo bloginfo('template_url' );?>/images/course-thumbnail.png" class="card-img-top" alt="...">
                                        <div class="card-body">
                                            <div class="card-subheading">
                                                <img src="<?php echo bloginfo('template_url' );?>/images/students-group-icon.png" alt="">
                                                <span>40+ Students</span>
                                            </div>
                                            <span class="date">1 - 28 July 2022</span>
                                            <h5 class="card-title">Workshop 2</h5>
                                            <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                                            <div class="d-flex align-items-center justify-content-between">
                                                <h6 class="color-orange">Duration: 2 Months</h6>
                                                <a href="#" class="btn btn-primary hvr-bob">Learn More</a>
                                            </div>

                                        </div>
                                    </div>
                                </div>
                                <div class="swiper-slide">
                                    <div class="card">
                                        <img src="<?php echo bloginfo('template_url' );?>/images/course-thumbnail.png" class="card-img-top" alt="...">
                                        <div class="card-body">
                                            <div class="card-subheading">
                                                <img src="<?php echo bloginfo('template_url' );?>/images/students-group-icon.png" alt="">
                                                <span>40+ Students</span>
                                            </div>
                                            <span class="date">1 - 28 July 2022</span>
                                            <h5 class="card-title">Workshop 3</h5>
                                            <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                                            <div class="d-flex align-items-center justify-content-between">
                                                <h6 class="color-orange">Duration: 2 Months</h6>
                                                <a href="#" class="btn btn-primary hvr-bob">Learn More</a>
                                            </div>

                                        </div>
                                    </div>
                                </div>
                                <div class="swiper-slide">
                                    <div class="card">
                                        <img src="<?php echo bloginfo('template_url' );?>/images/course-thumbnail.png" class="card-img-top" alt="...">
                                        <div class="card-body">
                                            <div class="card-subheading">
                                                <img src="<?php echo bloginfo('template_url' );?>/images/students-group-icon.png" alt="">
                                                <span>40+ Students</span>
                                            </div>
                                            <span class="date">1 - 28 July 2022</span>
                                            <h5 class="card-title">Workshop 4</h5>
                                            <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                                            <div class="d-flex align-items-center justify-content-between">
                                                <h6 class="color-orange">Duration: 2 Months</h6>
                                                <a href="#" class="btn btn-primary hvr-bob">Learn More</a>
                                            </div>

                                        </div>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <!-- #tab3 -->
                </div>
                <!-- .tab_container -->
            </div>
        </div>
    </div>
    </div>
</section>
<section class="section-padding-bottom section-methodology">
    <div class="container">
        <div class="row">
            <div class="col-lg-7 col-md-7 col-sm-12 col-xs-12 col-12 list-style pull-left">
                <h5 class="btn btn-sm color-primary px-3 hvr-fade bg-white-shade-1">Training</h5>
                <h2 class="color-primary">Our Teaching Methodology</h2>
                <div class="list-ul-content">
                    <ul>
                        <li>
                            <h4>Training</h4>
                            <p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium</p>
                        </li>
                        <li>
                            <h4>Training</h4>
                            <p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium</p>
                        </li>
                        <li>
                            <h4>Training</h4>
                            <p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium</p>
                        </li>
                        <li>
                            <h4>Training</h4>
                            <p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium</p>
                        </li>
                    </ul>
                </div>
            </div>
            <div class="col-lg-5 col-md-5 col-sm-12 col-xs-12 col-12 pull-left">
                <div class="rect-image">
                    <img src="<?php echo bloginfo('template_url' );?>/images/teaching-bg.png" alt="" />
                </div>
            </div>
        </div>
    </div>
</section>
<section class="section-padding section-methodology">
    <div class="container">
        <div class="row">
            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 col-12 pull-left">
                <div class="simple-mage">
                    <img src="<?php echo bloginfo('template_url' );?>/images/benifits.png" alt="" />
                </div>
            </div>
            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 col-12 list-style pull-left">
                <h5 class="btn btn-sm color-primary px-3 hvr-fade bg-primary-shade-1">Benefits</h5>
                <h2 class="color-orange">Why Choose us</h2>
                <div class="list-ul-content">
                    <ul>
                        <li>
                            <p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium</p>
                        </li>
                        <li>
                            <p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium</p>
                        </li>
                        <li>
                            <p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium</p>
                        </li>
                    </ul>
                </div>
            </div>

        </div>
    </div>
</section>
<section class="section-padding section-testimonial">
    <div class="container">
        <div class="row">
            <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 col-12 pull-left">
                <h5 class="btn btn-sm color-white px-3 hvr-fade btn-orange ">Happy Students</h5>
                <h2>What our students say about Us.</h2>
                <blockquote>Let the voice of our students speak about us. Their success and experiences are a testament to our commitment to excellence.</blockquote>
            </div>
            <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 col-12 list-style pull-left">

                <section class="slider testimonial-slider">
                    <div class="slider__flex">

                        <div class="slider__images">
                            <div class="swiper-container">
                                <div class="swiper-wrapper">
                                    <div class="swiper-slide">
                                        <div class="slider__image">
                                            <p>“On the Windows talking painted pasture yet its express parties use. Sure last upon he same as knew next. Of believed or diverted no.”</p>
                                            <div class="testimonial-author">
                                                <img src="<?php echo bloginfo('template_url' );?>/images/students.png" alt="" />
                                                <div class="testimonial-author-details">
                                                    <h4>Niresh Shresatha</h4>
                                                    <h5>Lincoln college</h5>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="swiper-slide">
                                        <div class="slider__image">
                                            <p>“On the Windows talking painted pasture yet its express parties use. Sure last upon he same as knew next. Of believed or diverted no.”</p>
                                            <div class="testimonial-author">
                                                <img src="<?php echo bloginfo('template_url' );?>/images/students.png" alt="" />
                                                <div class="testimonial-author-details">
                                                    <h4>Niresh Shresatha</h4>
                                                    <h5>Lincoln college</h5>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="swiper-slide">
                                        <div class="slider__image">
                                            <p>“On the Windows talking painted pasture yet its express parties use. Sure last upon he same as knew next. Of believed or diverted no.”</p>
                                            <div class="testimonial-author">
                                                <img src="<?php echo bloginfo('template_url' );?>/images/students.png" alt="" />
                                                <div class="testimonial-author-details">
                                                    <h4>Niresh Shresatha</h4>
                                                    <h5>Lincoln college</h5>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="swiper-slide">
                                        <div class="slider__image">
                                            <p>“On the Windows talking painted pasture yet its express parties use. Sure last upon he same as knew next. Of believed or diverted no.”</p>
                                            <div class="testimonial-author">
                                                <img src="<?php echo bloginfo('template_url' );?>/images/students.png" alt="" />
                                                <div class="testimonial-author-details">
                                                    <h4>Niresh Shresatha</h4>
                                                    <h5>Lincoln college</h5>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="swiper-slide">
                                        <div class="slider__image">
                                            <p>“On the Windows talking painted pasture yet its express parties use. Sure last upon he same as knew next. Of believed or diverted no.”</p>
                                            <div class="testimonial-author">
                                                <img src="<?php echo bloginfo('template_url' );?>/images/students.png" alt="" />
                                                <div class="testimonial-author-details">
                                                    <h4>Niresh Shresatha</h4>
                                                    <h5>Lincoln college</h5>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="slider__col">
                            <div class="slider__prev"><i class="fa-solid fa-angle-up"></i></div>

                            <div class="slider__next"><i class="fa-solid fa-angle-down"></i></div>
                        </div>
                    </div>
                </section>
            </div>

        </div>
    </div>
</section>
<section class="section-padding section-tools-marquee">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <div class="swiper-container swiper--bottom">
                    <div class="swiper-wrapper">
                        <div class="swiper-slide">
                            <div class="tools-marquee">
                                <img src="<?php echo bloginfo('template_url' );?>/images/aftereffects.jpg" alt="" />
                                <h3>After effects</h3>
                            </div>
                        </div>
                        <div class="swiper-slide">
                            <div class="tools-marquee">
                                <img src="<?php echo bloginfo('template_url' );?>/images/indesign.jpg" alt="" />
                                <h3>Indesign</h3>
                            </div>
                        </div>
                        <div class="swiper-slide">
                            <div class="tools-marquee">
                                <img src="<?php echo bloginfo('template_url' );?>/images/mediaencoder.jpg" alt="" />
                                <h3>After effects</h3>
                            </div>
                        </div>
                        <div class="swiper-slide">
                            <div class="tools-marquee">
                                <img src="<?php echo bloginfo('template_url' );?>/images/mspackage.jpg" alt="" />
                                <h3>After effects</h3>
                            </div>
                        </div>
                        <div class="swiper-slide">
                            <div class="tools-marquee">
                                <img src="<?php echo bloginfo('template_url' );?>/images/premoere.jpg" alt="" />
                                <h3>After effects</h3>
                            </div>
                        </div>
                        <div class="swiper-slide">
                            <div class="tools-marquee">
                                <img src="<?php echo bloginfo('template_url' );?>/images/photoshop.png" alt="" />
                                <h3>After effects</h3>
                            </div>
                        </div>
                        <div class="swiper-slide">
                            <div class="tools-marquee">
                                <img src="<?php echo bloginfo('template_url' );?>/images/illustrator.png" alt="" />
                                <h3>After effects</h3>
                            </div>
                        </div>
                        <div class="swiper-slide">
                            <div class="tools-marquee">
                                <img src="<?php echo bloginfo('template_url' );?>/images/bootstrap.png" alt="" />
                                <h3>After effects</h3>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<?php get_footer();?>
